  ...
  const submit = async () => {
    ...
    if (userId) {
      try {
        ...
        // Update the state with the new task, avoid adding duplicate tasks
        setTasks((prevTasks) => {
          // Avoid duplicates by checking task titles or any unique identifier
          const taskExists = prevTasks.some((task) => task.title === newTask.title);
          if (!taskExists) {
            return [...prevTasks, response.data.list]; // Assuming server returns the saved task
          }
          return prevTasks; // Don't add the task if it already exists
        });
